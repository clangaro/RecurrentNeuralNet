...
def simulate_trial(
    ...
    """    
    Simulate a single trial of the network dynamics using forwards Euler integration.
...
for k in range(n_steps):
    ...
    if k % C.record_every == 0:
        t_recorded[record.idx] = k * C.dt
        r_e_recorded[record.idx, :] = r_e
        r_i_recorded[record.idx, :] = r_i
        s_e_recorded[record.idx, :] = s_e
        record.idx += 1

return Trajectory(t=t_recorded, 
                    r_e=r_e_recorded, 
                    r_i=r_i_recorded, 
                    s_e=s_e_recorded)
...
def make_pulse_train(
    ...
    Create a pulse train input vector in discrete time.
...
#US ALSO DRIVES INHIBITION -- feedforward inhibition mechanism placeholder 
u_i[:, :] = us_i[:, None]
